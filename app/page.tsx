...
import PulsePanel from './sections/PulsePanel'
...
interface Branch { _id?: string; name: string; repo_owner: string; repo_name: string; author?: string; status?: string; files_changed?: string[]; commit_count?: number; last_commit_message?: string; last_commit_date?: string; assigned_member?: string; description?: string; pulse_count?: number; createdAt?: string }
interface PulseEvent { _id?: string; branch_id: string; branch_name: string; event_type: string; message: string; author?: string; severity?: string; metadata?: Record<string, any>; createdAt?: string }
...
function AppContent() {
  ...
  const fetchPulseEvents = useCallback(async (branchId?: string) => {
    setPulseLoading(true)
    try {
      const url = branchId ? `/api/pulse?branch_id=${branchId}` : '/api/pulse'
      const res = await fetch(url)
      const data = await res.json()
      if (data?.success && Array.isArray(data.data)) setPulseEvents(data.data)
    } catch {}
    setPulseLoading(false)
  }, [])

  const handleCreateBranch = async (branch: Partial<Branch>) => {
    try {
      const res = await fetch('/api/branches', {
        method: 'POST',
  ...
  const handleJoinBranch = async (branchId: string, memberName: string) => {
    try {
      const res = await fetch(`/api/branches/${branchId}/join`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ member_name: memberName }),
      })
      const data = await res.json()
      if (data?.success) { fetchBranches(); fetchPulseEvents(branchId) }
    } catch {}
  }

  const handleHandoffBranch = async (branchId: string, fromMember: string, toMember: string, notes: string) => {
    try {
      const res = await fetch(`/api/branches/${branchId}/handoff`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ from_member: fromMember, to_member: toMember, notes }),
      })
      const data = await res.json()
      if (data?.success) { fetchBranches(); fetchPulseEvents(branchId) }
    } catch {}
  ...
  const handleAddPulseEvent = async (event: { branch_id: string; branch_name: string; event_type: string; message: string; author: string; severity: string }) => {
    try {
      ...
      const data = await res.json()
      if (data?.success) fetchPulseEvents(event.branch_id)
    } catch {}
  }
  ...
  const handleSyncFromGithub = async () => {
    setSyncLoading(true)
    try {
      const owner = githubSettings.repo_owner
      const repo = githubSettings.repo_name
      const token = githubSettings.github_token_ref
      if (!owner || !repo || !token) { setSyncLoading(false); return }
      const res = await fetch(`/api/github/context?owner=${encodeURIComponent(owner)}&repo=${encodeURIComponent(repo)}&token=${encodeURIComponent(token)}`)
      const data = await res.json()
      if (res.ok && Array.isArray(data.branches)) {
        for (const gb of data.branches) {
          const exists = branches.find((b) => b.name === gb.name)
          if (!exists) {
            await fetch('/api/branches', {
              ...
              body: JSON.stringify({
                ...
                author: gb.author || '',
                status: 'active',
                files_changed: Array.isArray(gb.files) ? gb.files : [],
                commit_count: 0,
              }),
            })
          }
  ...
  useEffect(() => {
    ...
    fetchBranches()
    fetchPulseEvents()
  }, [fetchTeamMembers, fetchDecisions, fetchGithubSettings, fetchBranches, fetchPulseEvents])

  useEffect(() => {
    if (selectedBranchId) fetchPulseEvents(selectedBranchId)
  }, [selectedBranchId, fetchPulseEvents])
  ...
  return (
    <div className="flex min-h-screen">
      <LeftRail activePage={activePage} onNavigate={setActivePage} branchCount={branches.length} pulseCount={pulseEvents.length} />
      ...
      {activePage === 'branches' ? (
            ...
            {selectedBranch && (
              <div className="w-80 border-l border-border shrink-0 overflow-y-auto">
                <PulsePanel
                  ...
                  onAddEvent={handleAddPulseEvent}
        ...
        <main className="flex-1 min-w-0 overflow-y-auto">
          ...
          <div className="p-6 max-w-4xl mx-auto">
            <PulsePanel events={pulseEvents} loading={pulseLoading} />
          </div>
        </main>
      ) : (
        <main className="flex-1 min-w-0 overflow-y-auto">
