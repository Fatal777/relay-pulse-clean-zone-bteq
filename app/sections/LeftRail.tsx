...
import { Home, FileText, GitPullRequest, Settings, GitBranch, Activity } from 'lucide-react'
...
export default function LeftRail({ activePage, onNavigate, branchCount, pulseCount }: LeftRailProps) {
  return (
    <aside className="w-16 lg:w-56 min-h-screen border-r border-border flex flex-col py-6 px-2 lg:px-4 shrink-0" style={{ background: 'hsl(231, 18%, 12%)' }}>
      ...
      <nav className="flex flex-col gap-1">
        {navItems.map((item) => {
          ...
          const badge = item.id === 'branches' ? branchCount : item.id === 'pulse' ? pulseCount : undefined
          return (
            <button
              key={item.id}
            ...
            >
              ...
              {badge !== undefined && badge > 0 && (
                <span className="hidden lg:inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-bold rounded-full bg-primary/20 text-primary">
                  {badge}
                </span>
              )}
            </button>
