...
import { FiRefreshCw, FiAlertTriangle, FiGitBranch, FiActivity, FiUser } from 'react-icons/fi'
                  ...
                  {displayActivity.map((item, i) => (
                    <div key={i} className="flex items-start gap-3 p-2 rounded-lg hover:bg-secondary/30 transition-colors">
                      <div className="w-7 h-7 rounded-full bg-primary/15 flex items-center justify-center mt-0.5 shrink-0">
                        {item.type === 'commit' && <FiActivity className="w-3.5 h-3.5 text-primary" />}
                        {item.type === 'pr' && <FiGitBranch className="w-3.5 h-3.5 text-green-400" />}
                        {item.type === 'branch' && <FiGitBranch className="w-3.5 h-3.5 text-yellow-400" />}
                      </div>
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" variant="outline" onClick={onAnalyzeConflicts} disabled={conflictLoading} className="text-xs">
              {conflictLoading ? <><Loader2 className="w-3 h-3 mr-1.5 animate-spin" />Analyzing...</> : <><FiRefreshCw className="w-3 h-3 mr-1.5" />Analyze Conflicts</>}
            </Button>
          </div>
        </CardHeader>
        <CardContent>
