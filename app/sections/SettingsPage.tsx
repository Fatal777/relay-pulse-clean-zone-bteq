...
import { FiGithub, FiUser, FiPlus, FiTrash2, FiCheck } from 'react-icons/fi'
      ...
      <Card className="border-border bg-card shadow-xl">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            <FiGithub className="w-4 h-4 text-foreground" />
            GitHub Connection
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleSaveGithub} disabled={settingsLoading || !ghForm.repo_owner.trim() || !ghForm.repo_name.trim()}>
            {settingsLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Saving...</> : ghSaved ? <><FiCheck className="w-4 h-4 mr-2" />Saved</> : 'Save Settings'}
          </Button>
        </CardContent>
      </Card>
